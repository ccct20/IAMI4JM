#coding=utf-8

from collections import defaultdict
import numpy as np
from time import time
import random
from ipdb import set_trace


class DataModule():
    def __init__(self, conf, filename, pop_path=None):
        self.conf = conf
        self.data_dict = {}
        self.terminal_flag = 1
        self.s_terminal_flag = 1
        self.filename = filename
        self.index = 0
        self.pop_path = pop_path
        self.margin = self.conf.r_margin
        self.s_margin = self.conf.s_margin

###########################################  Initalize Procedures ############################################
    def prepareModelSupplement(self, model):
        data_dict = {}
        if 'CONSUMED_ITEMS_SPARSE_MATRIX' in model.supply_set:
            self.generateConsumedItemsSparseMatrix()
            data_dict['CONSUMED_ITEMS_INDICES_INPUT'] = self.consumed_items_indices_list
            data_dict['CONSUMED_ITEMS_VALUES_INPUT'] = self.consumed_items_values_list
        return data_dict

    def prepareModelSocialSupplement(self, model):
        data_dict = {}
        if 'SOCIAL_NEIGHBORS_SPARSE_MATRIX' in model.supply_set:
            self.readSocialNeighbors()
            self.generateSocialNeighborsSparseMatrix()
            data_dict['SOCIAL_NEIGHBORS_INDICES_INPUT'] = self.social_neighbors_indices_list
            data_dict['SOCIAL_NEIGHBORS_VALUES_INPUT'] = self.social_neighbors_values_list
        return data_dict

    # RANKING
    def initializeRankingTrain(self):
        self.readData()
        self.generate_popularity() #add
        self.arrangePositiveData()
        self.generateTrainNegative()

    def initializeRankingVT(self):
        self.readData()
        self.generate_popularity() #add
        self.arrangePositiveData()
        self.generateTrainNegative()

    def initalizeRankingEva(self):
        self.readData()
        self.generate_popularity() #add
        self.getEvaPositiveBatch()
        self.generateEvaNegative()

    def linkedMap(self):
        self.data_dict['POP_USER_LIST'] = self.pop_user_list
        self.data_dict['POP_ITEM_LIST'] = self.pop_item_list
        self.data_dict['POP_LABEL_LIST'] = self.pop_labels_list

        self.data_dict['UNPOP_USER_LIST'] = self.unpop_user_list
        self.data_dict['UNPOP_ITEM_LIST'] = self.unpop_item_list
        self.data_dict['UNPOP_LABEL_LIST'] = self.unpop_labels_list
    
    def linkedRankingEvaMap(self):
        self.data_dict['EVA_USER_LIST'] = self.eva_user_list
        self.data_dict['EVA_ITEM_LIST'] = self.eva_item_list

    # SOCIAL
    def initializeSocialTrain(self):
        self.readSocialData()
        self.generate_social_popularity()
        self.arrangePositiveSocialData()
        self.generateSocialTrainNegative()

    def initializeSocialVT(self):
        self.readSocialData()
        self.generate_social_popularity()
        self.arrangePositiveSocialData()
        self.generateSocialTrainNegative()

    def initalizeSocialEva(self):
        self.readSocialData()
        self.generate_social_popularity()
        self.getEvaSocialPositiveBatch()
        self.generateSocialEvaNegative()

    def linkedMapSocial(self):
        self.data_dict['POP_USER1_LIST'] = self.pop_user1_list
        self.data_dict['POP_USER2_LIST'] = self.pop_user2_list
        self.data_dict['POP_SOCIAL_LABEL_LIST'] = self.pop_s_labels_list
        self.data_dict['UNPOP_USER1_LIST'] = self.unpop_user1_list
        self.data_dict['UNPOP_USER2_LIST'] = self.unpop_user2_list
        self.data_dict['UNPOP_SOCIAL_LABEL_LIST'] = self.unpop_s_labels_list

    def linkedSocialEvaMap(self):
        self.data_dict['EVA_USER1_LIST'] = self.eva_user1_list
        self.data_dict['EVA_USER2_LIST'] = self.eva_user2_list

###########################################  Ranking ############################################
    def readData(self):
        f = open(self.filename) ## May should be specific for different subtasks
        total_user_list = set()
        hash_data = defaultdict(int)
        for _, line in enumerate(f):
            arr = line.split("\t")
            hash_data[(int(arr[0]), int(arr[1]))] = 1
            total_user_list.add(int(arr[0]))
        self.total_user_list = list(total_user_list)
        self.hash_data = hash_data
    

    def generate_popularity(self):
        try:
            self.popularity = np.load(self.pop_path, allow_pickle=True)
        except IOError:
            popularity = np.zeros(self.conf.num_items, dtype=np.int32)
            for (u, i) in self.hash_data:
                popularity[i] += 1
            self.popularity = popularity
            np.save(self.pop_path, popularity)


    def arrangePositiveData(self):
        positive_data = defaultdict(set)
        total_data = set()
        hash_data = self.hash_data
        for (u, i) in hash_data:
            total_data.add((u, i))
            positive_data[u].add(i)
        self.positive_data = positive_data
    
    '''
        This function designes for the train/val/test negative generating section
    '''


    def generateTrainNegative(self):
        num_items = self.conf.num_items
        num_negatives = self.conf.num_negatives 
        loop_num = self.conf.r_loop_num
        negative_data = defaultdict(set)
        total_data = defaultdict(set)
        popularity = self.popularity
        hash_data = self.hash_data
        total_p_count = 0
        total_n_count = 0

        for (u, i) in hash_data:
            item_pos_pop = popularity[i]
            count = 0
            while count < num_negatives:
                for target_mask in [True, False]:  # True: 更流行, False: 更不流行
                    for _ in range(loop_num):  # 最多尝试100次
                        j = np.random.randint(num_items)
                        if (u, j) in hash_data:
                            continue
                        sample_popularity = popularity[j]
                        if target_mask and sample_popularity >= item_pos_pop + self.margin:
                            negative_data[u].add((j, True))
                            total_data[u].add((i, j, True))
                            total_p_count += 1
                            break
                        elif not target_mask and sample_popularity <= item_pos_pop - self.margin:
                            negative_data[u].add((j, False))
                            total_data[u].add((i, j, False))
                            total_n_count += 1
                            break
                    count += 1

        print("负样本流行数:%s, 不流行数:%s" %(total_p_count, total_n_count))
        self.total_data = total_data
        self.negative_data = negative_data
        self.terminal_flag = 1
        
    '''
        This function designes for the val/test section, compute loss
    '''
    def getVTRankingOneBatch(self):
        positive_data = self.positive_data
        total_data = self.total_data
        negative_data = self.negative_data
        total_user_list = self.total_user_list
        pop_user_list = []
        pop_item_list = []
        pop_labels_list = []
        unpop_user_list, unpop_item_list, unpop_labels_list = [], [], []
        
        for u in total_user_list:
            pop_user_list.extend([u] * len(positive_data[u]))
            unpop_user_list.extend([u] * len(positive_data[u]))
            pop_item_list.extend(positive_data[u])
            unpop_item_list.extend(positive_data[u])
            pop_labels_list.extend([1] * len(positive_data[u]))
            unpop_labels_list.extend([1] * len(positive_data[u]))

            for j, mask in negative_data[u]:
                if mask == 1:
                    pop_user_list.extend([u])
                    pop_item_list.extend([j])
                    pop_labels_list.extend([0])
                elif mask == 0:
                    unpop_user_list.extend([u])
                    unpop_item_list.extend([j])
                    unpop_labels_list.extend([0])
        
        self.pop_user_list = np.reshape(pop_user_list, [-1, 1])
        self.pop_item_list = np.reshape(pop_item_list, [-1, 1])
        self.pop_labels_list = np.reshape(pop_labels_list, [-1, 1])
        self.unpop_user_list = np.reshape(unpop_user_list, [-1, 1])
        self.unpop_item_list = np.reshape(unpop_item_list, [-1, 1])
        self.unpop_labels_list = np.reshape(unpop_labels_list, [-1, 1])
    
    '''
        This function designes for the training process
    '''
    def getTrainRankingBatch(self):
        positive_data = self.positive_data
        negative_data = self.negative_data
        total_data = self.total_data
        total_user_list = self.total_user_list
        index = self.index
        batch_size = self.conf.training_batch_size

        pop_user_list, pop_item_list, pop_labels_list = [], [], []
        unpop_user_list, unpop_item_list, unpop_labels_list = [], [], []
        
        if index + batch_size < len(total_user_list):
            target_user_list = total_user_list[index:index+batch_size]
            self.index = index + batch_size
        else:
            target_user_list = total_user_list[index:len(total_user_list)]
            self.index = 0
            self.terminal_flag = 0

        for u in target_user_list:
            pop_user_list.extend([u] * len(positive_data[u]))
            unpop_user_list.extend([u] * len(positive_data[u]))
            pop_item_list.extend(positive_data[u])
            unpop_item_list.extend(positive_data[u])
            pop_labels_list.extend([1] * len(positive_data[u]))
            unpop_labels_list.extend([1] * len(positive_data[u]))

            for j, mask in negative_data[u]:
                if mask == 1:
                    pop_user_list.extend([u])
                    pop_item_list.extend([j])
                    pop_labels_list.extend([0])
                elif mask == 0:
                    unpop_user_list.extend([u])
                    unpop_item_list.extend([j])
                    unpop_labels_list.extend([0])
        
        self.pop_user_list = np.reshape(pop_user_list, [-1, 1])
        self.pop_item_list = np.reshape(pop_item_list, [-1, 1])
        self.pop_labels_list = np.reshape(pop_labels_list, [-1, 1])
        self.unpop_user_list = np.reshape(unpop_user_list, [-1, 1])
        self.unpop_item_list = np.reshape(unpop_item_list, [-1, 1])
        self.unpop_labels_list = np.reshape(unpop_labels_list, [-1, 1])
    
    '''
        This function designes for the positive data in rating evaluate section
    '''
    def getEvaPositiveBatch(self):
        hash_data = self.hash_data
        user_list = []
        item_list = []
        index_dict = defaultdict(list)
        index = 0
        for (u, i) in hash_data:
            user_list.append(u)
            item_list.append(i)
            index_dict[u].append(index)
            index = index + 1
        self.eva_user_list = np.reshape(user_list, [-1, 1])
        self.eva_item_list = np.reshape(item_list, [-1, 1])
        self.eva_index_dict = index_dict

    '''
        This function designes for the negative data generation process in rating evaluate section
    '''
    def generateEvaNegative(self):
        hash_data = self.hash_data
        total_user_list = self.total_user_list
        num_evaluate = self.conf.num_evaluate
        num_items = self.conf.num_items
        eva_negative_data = defaultdict(list)
        for u in total_user_list:
            for _ in range(num_evaluate):
                j = np.random.randint(num_items)
                while (u, j) in hash_data:
                    j = np.random.randint(num_items)
                eva_negative_data[u].append(j)
        self.eva_negative_data = eva_negative_data

    '''
        This function designs for the rating evaluate section, generate negative batch
    '''
    def getEvaRankingBatch(self):
        batch_size = self.conf.evaluate_batch_size
        num_evaluate = self.conf.num_evaluate
        eva_negative_data = self.eva_negative_data
        total_user_list = self.total_user_list
        index = self.index
        terminal_flag = 1
        total_users = len(total_user_list)
        user_list = []
        item_list = []
        if index + batch_size < total_users:
            batch_user_list = total_user_list[index:index+batch_size]
            self.index = index + batch_size
        else:
            terminal_flag = 0
            batch_user_list = total_user_list[index:total_users]
            self.index = 0
        for u in batch_user_list:
            user_list.extend([u]*num_evaluate)
            item_list.extend(eva_negative_data[u])
        self.eva_user_list = np.reshape(user_list, [-1, 1])
        self.eva_item_list = np.reshape(item_list, [-1, 1])
        return batch_user_list, terminal_flag


###########################################  Social ############################################
    def readSocialData(self):
        f = open(self.filename) ## May should be specific for different subtasks
        social_total_user1_list = set()
        social_total_user2_list = set()
        social_hash_data = defaultdict(int)
        for _, line in enumerate(f):
            arr = line.strip().split()
            social_hash_data[(int(arr[0]), int(arr[1]))] = 1
            social_total_user1_list.add(int(arr[0]))
            social_total_user2_list.add(int(arr[1]))
        self.social_total_user1_list = list(social_total_user1_list)
        self.social_total_user2_list = list(social_total_user2_list)
        self.social_hash_data = social_hash_data

    def generate_social_popularity(self):
        try:
            self.social_popularity = np.load(self.pop_path, allow_pickle=True)
        except IOError:
            popularity = np.zeros(self.conf.num_users, dtype=np.int32)
            for (u1, u2) in self.social_hash_data:
                popularity[u2] += 1
                popularity[u1] += 1
            self.social_popularity = popularity
            np.save(self.pop_path, popularity)

    def arrangePositiveSocialData(self):
        social_positive_data = defaultdict(set)
        social_total_data = set()
        social_hash_data = self.social_hash_data
        for (u1, u2) in social_hash_data:
            social_total_data.add((u1, u2))
            social_positive_data[u1].add(u2)
        self.social_positive_data = social_positive_data
        self.social_total_data = len(social_total_data)

    def generateSocialTrainNegative(self):
        num_users = self.conf.num_users
        num_social_negatives = self.conf.num_social_negatives 
        loop_num = self.conf.s_loop_num
        social_negative_data = defaultdict(set)
        social_total_data = defaultdict(set)
        social_hash_data = self.social_hash_data
        popularity = self.social_popularity
        total_p_count = 0
        total_n_count = 0
        for (u1, u2) in social_hash_data:
            u2_pos_pop = popularity[u2]
            count = 0
            while count < num_social_negatives:
                for target_mask in [True, False]:  # True: 更流行, False: 更不流行
                    for _ in range(loop_num):
                        j = np.random.randint(num_users)
                        if (u1, j) in social_hash_data:
                            continue
                        sample_popularity = popularity[j]
                        if target_mask and sample_popularity >= u2_pos_pop + self.s_margin:
                            social_negative_data[u1].add((j, True))
                            social_total_data[u1].add((u2, j, True))
                            total_p_count += 1
                            break
                        elif not target_mask and sample_popularity <= u2_pos_pop - self.s_margin:
                            social_negative_data[u1].add((j, False))
                            social_total_data[u1].add((u2, j, False))
                            total_n_count += 1
                            break
                    count += 1

        print("Social负样本流行数:%s, 不流行数:%s" %(total_p_count, total_n_count))
        self.social_negative_data = social_negative_data
        self.social_total_data = social_total_data
        self.s_terminal_flag = 1


    def getVTSocialOneBatch(self):
        social_positive_data = self.social_positive_data
        social_negative_data = self.social_negative_data
        social_total_data = self.social_total_data
        social_total_user1_list = self.social_total_user1_list
        pop_user1_list, pop_user2_list, pop_s_labels_list = [], [], []
        unpop_user1_list, unpop_user2_list, unpop_s_labels_list = [], [], []
        for u1 in social_total_user1_list:
            for u2, j, mask in social_total_data[u1]:
                if mask == 1:
                    pop_user1_list.extend([u1]*2)
                    pop_user2_list.append(u2)
                    pop_s_labels_list.append(1)
                    pop_user2_list.append(j)
                    pop_s_labels_list.append(0)
                elif mask == 0:
                    unpop_user1_list.extend([u1]*2)
                    unpop_user2_list.append(u2)
                    unpop_s_labels_list.append(1)
                    unpop_user2_list.append(j)
                    unpop_s_labels_list.append(0)

        self.pop_user1_list = np.reshape(pop_user1_list, [-1, 1])
        self.pop_user2_list = np.reshape(pop_user2_list, [-1, 1])
        self.pop_s_labels_list = np.reshape(pop_s_labels_list, [-1, 1])
        self.unpop_user1_list = np.reshape(unpop_user1_list, [-1, 1])
        self.unpop_user2_list = np.reshape(unpop_user2_list, [-1, 1])
        self.unpop_s_labels_list = np.reshape(unpop_s_labels_list, [-1, 1])

    def getTrainSocialBatch(self):
        social_positive_data = self.social_positive_data
        social_negative_data = self.social_negative_data
        social_total_data = self.social_total_data
        social_total_user1_list = self.social_total_user1_list
        index = self.index
        batch_size = self.conf.training_batch_size

        pop_user1_list, pop_user2_list, pop_s_labels_list = [], [], []
        unpop_user1_list, unpop_user2_list, unpop_s_labels_list = [], [], []
        
        if index + batch_size < len(social_total_user1_list):
            target_user_list = social_total_user1_list[index:index+batch_size]
            self.index = index + batch_size
        else:
            target_user_list = social_total_user1_list[index:len(social_total_user1_list)]
            self.index = 0
            self.s_terminal_flag = 0

        for u1 in target_user_list:
            for u2, j, mask in social_total_data[u1]:
                if mask == 1:
                    pop_user1_list.extend([u1]*2)
                    pop_user2_list.append(u2)
                    pop_s_labels_list.append(1)
                    pop_user2_list.append(j)
                    pop_s_labels_list.append(0)
                elif mask == 0:
                    unpop_user1_list.extend([u1]*2)
                    unpop_user2_list.append(u2)
                    unpop_s_labels_list.append(1)
                    unpop_user2_list.append(j)
                    unpop_s_labels_list.append(0)

        self.pop_user1_list = np.reshape(pop_user1_list, [-1, 1])
        self.pop_user2_list = np.reshape(pop_user2_list, [-1, 1])
        self.pop_s_labels_list = np.reshape(pop_s_labels_list, [-1, 1])
        self.unpop_user1_list = np.reshape(unpop_user1_list, [-1, 1])
        self.unpop_user2_list = np.reshape(unpop_user2_list, [-1, 1])
        self.unpop_s_labels_list = np.reshape(unpop_s_labels_list, [-1, 1])

    def getEvaSocialPositiveBatch(self):
        social_hash_data = self.social_hash_data
        user1_list = []
        user2_list = []
        index_dict = defaultdict(list)
        index = 0
        for (u1, u2) in social_hash_data:
            user1_list.append(u1)
            user2_list.append(u2)
            index_dict[u1].append(index)
            index = index + 1
        self.eva_user1_list = np.reshape(user1_list, [-1, 1])
        self.eva_user2_list = np.reshape(user2_list, [-1, 1])
        self.eva_social_index_dict = index_dict

    def generateSocialEvaNegative(self):
        social_hash_data = self.social_hash_data
        social_total_user1_list = self.social_total_user1_list
        num_social_evaluate = self.conf.num_social_evaluate
        num_users = self.conf.num_users
        eva_social_negative_data = defaultdict(list)
        for u1 in social_total_user1_list:
            for _ in range(num_social_evaluate):
                j = np.random.randint(num_users)
                while (u1, j) in social_hash_data:
                    j = np.random.randint(num_users)
                eva_social_negative_data[u1].append(j)
        self.eva_social_negative_data = eva_social_negative_data

    def getEvaSocialBatch(self):
        batch_size = self.conf.evaluate_batch_size
        num_social_evaluate = self.conf.num_social_evaluate
        eva_social_negative_data = self.eva_social_negative_data
        social_total_user1_list = self.social_total_user1_list
        index = self.index
        terminal_flag = 1
        social_total_users = len(social_total_user1_list)
        user1_list = []
        user2_list = []
        if index + batch_size < social_total_users:
            batch_user_list = social_total_user1_list[index:index+batch_size]
            self.index = index + batch_size
        else:
            terminal_flag = 0
            batch_user_list = social_total_user1_list[index:social_total_users]
            self.index = 0
        for u1 in batch_user_list:
            user1_list.extend([u1]*num_social_evaluate)
            user2_list.extend(eva_social_negative_data[u1])
        self.eva_user1_list = np.reshape(user1_list, [-1, 1])
        self.eva_user2_list = np.reshape(user2_list, [-1, 1])
        return batch_user_list, terminal_flag

##################################################### Supplement for Sparse Computation ############################################
    def readSocialNeighbors(self, friends_flag=1):
        social_neighbors = defaultdict(set)
        links_file = open(self.conf.links_filename)
        for _, line in enumerate(links_file):
            tmp = line.split('\t')
            u1, u2 = int(tmp[0]), int(tmp[1])
            social_neighbors[u1].add(u2)
            if friends_flag == 1:
                social_neighbors[u2].add(u1)
        self.social_neighbors = social_neighbors

    '''
        Generate Social Neighbors Sparse Matrix Indices and Values
    '''
    def generateSocialNeighborsSparseMatrix(self):
        social_neighbors = self.social_neighbors
        social_neighbors_indices_list = []
        social_neighbors_values_list = []
        social_neighbors_dict = defaultdict(list)
        for u in social_neighbors:
            social_neighbors_dict[u] = sorted(social_neighbors[u])
        user_list = sorted(list(social_neighbors.keys()))
        for user in user_list:
            for friend in social_neighbors_dict[user]:
                social_neighbors_indices_list.append([user, friend])
                social_neighbors_values_list.append(1.0/len(social_neighbors_dict[user]))
        self.social_neighbors_indices_list = np.array(social_neighbors_indices_list).astype(np.int64)
        self.social_neighbors_values_list = np.array(social_neighbors_values_list).astype(np.float32)
    
    '''
        Generate Consumed Items Sparse Matrix Indices and Values
    '''
    def generateConsumedItemsSparseMatrix(self):
        positive_data = self.positive_data  
        consumed_items_indices_list = []
        consumed_items_values_list = []
        consumed_items_dict = defaultdict(list)
        for u in positive_data:
            consumed_items_dict[u] = sorted(positive_data[u])
        user_list = sorted(list(positive_data.keys()))
        for u in user_list:
            for i in consumed_items_dict[u]:
                consumed_items_indices_list.append([u, i])
                consumed_items_values_list.append(1.0/len(consumed_items_dict[u]))
        self.consumed_items_indices_list = np.array(consumed_items_indices_list).astype(np.int64)
        self.consumed_items_values_list = np.array(consumed_items_values_list).astype(np.float32)
